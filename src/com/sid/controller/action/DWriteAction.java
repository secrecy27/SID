...
public class DWriteAction implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		dVo.setPoint(multi.getParameter("point"));
		dVo.setExpl(multi.getParameter("expl"));
		dVo.setUserEmail((String)session.getAttribute("email"));
		System.out.println();
		DPageDAO dao = DPageDAO.getInstance();
		int result = dao.insertImage(dVo);

		if (result > 0) {
			System.out.println("d write success");
		} else {
			System.out.println("d write fail");
		}
		
		RequestDispatcher dispatcher = request.getRequestDispatcher(url);
		dispatcher.forward(request, response);
>>>>>>> branch 'master' of https://github.com/secrecy27/SID.git
	}
}
